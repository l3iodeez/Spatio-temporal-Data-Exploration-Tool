var AdvancedD3Chart = React.createClass({
  ...
  drawChart: function(filterExtents) {
    ...
    var xScale = d3.time.scale().range([0, sizes.width]);

    var xScale2 = d3.time.scale().range([0, sizes.width]); // Duplicate xScale for brushing ref later

    var yScale = d3.scale.linear().range([sizes.height, 0]);
    ...
    var svg = d3
      .select("#visualisation" + this.props.className.split(" ").join(""))
      .append("svg")
    ...
    brush.x(xScale2).on("brush", this.redrawDataLines);
    ...
    var legend = svg.append("g").attr("class", "legend-container");
    var dataLine = legend
      .selectAll(".dataLine")
    ...
    dataLine
      ...
      .on(
        ...
        function(d) {
          ...
          this.drawCombinedTrendline(svg, xScale, yScale);
    ...
    var columnNames = d3
      .keys(data[0]) //grab the key values from the first data row
  ...
  findMaxX: function() {
    var maxXValues = this.state.series.map(function(d) {
      if (d.visible) {
        return d3.max(d.values, function(dataPoint) {
  ...
  findMinX: function() {
    var minXValues = this.state.series.map(function(d) {
      if (d.visible) {
        return d3.min(d.values, function(dataPoint) {
          ...
          var xSeries = [].concat
            .apply(
              ...
              series.values.map(
                function(d) {
                  if (
                    d.measureDate >= filterStart &&
                    d.measureDate <= filterEnd
                  ) {
  ...
  drawAveragedTrendline: function(svg, xScale, yScale) {
    ...
    var run =
      ...
      this.timeResolutionOptions()[this.state.timeResolution].value();

    var rise = run * averagedLine[0];

    var midY =
      (this.state.yAxis.scale().domain()[1] -
        this.state.yAxis.scale().domain()[0]) /
      2;
    ...
    svg
      ...
      .attr("y1", yScale(averagedLine[3] - rise / 2))
      .attr("x2", xScale(this.state.filterEnd))
      .attr("y2", yScale(averagedLine[3] + rise / 2))
      .attr("stroke", "black")
      .attr("stroke-width", 3);
    ...
    svg
      ...
      .text(
        "eq: " +
          averagedLine[0].toExponential(3) +
          "x + " +
  ...
  drawCombinedTrendline: function(svg, xScale, yScale) {
        ...
        this.state.series.map(function(s) {
          if (s.visible) {
            return s.values.map(function(d) {
              if (
                d.measureDate >= xScale.domain()[0] &&
                d.measureDate <= xScale.domain()[1]
              ) {
                return d.measureDate;
              }
            });
    ...
    svg
      ...
      .text(
        "eq: " +
          decimalFormat(leastSquaresCoeff[0]) +
          "x + " +
  ...
  render: function() {
    return (
      <div
        id={"adv-d3-container" + this.props.className.split(" ").join("")}
        className="graph"
      >
        <div
        ...
        <button disabled={!this.state.series} onClick={this.saveDisplayed}>
          Save Displayed as:
        </button>
        {this.state.series ? (
          <div>
            ...
            >
              {Object.keys(this.timeResolutionOptions()).map(
                function(option) {
                  return (
                    <option key={option} value={option}>
                      {this.timeResolutionOptions()[option].display}
                    </option>
                  );
                }.bind(this)
              )}
