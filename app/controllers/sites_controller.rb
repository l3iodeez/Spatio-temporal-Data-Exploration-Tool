class SitesController < ApplicationController
 ...
 @sites = Site.where("state = 'NJ' and measure_count > 10000")
...
def geocode_all
  ...
  # GET /sites/1.json
  def show
     @sites = Site.near("#{@site.address} #{@site.city}, #{@site.state}",100).where("state = 'NJ' and measure_count > 10000") #Site.where("state = 'NJ' and measure_count > 1000")
      @hash = Gmaps4rails.build_markers(@sites) do |site, marker|
      if site.measure_count
        site_link = view_context.link_to site.id.to_s, site_path(site.id)
        marker.lat site.latitude
